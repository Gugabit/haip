...
async function main() {
  ...
  const enums = [
    ...
    `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'payment_method') THEN CREATE TYPE payment_method AS ENUM ('credit_card','debit_card','card_machine','pix','cash','bank_transfer','city_ledger','vcc','other'); END IF; END $$`,
    // Idempotent add: append card_machine / pix to payment_method if it already existed without them
    `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid = e.enumtypid WHERE t.typname = 'payment_method' AND e.enumlabel = 'card_machine') THEN ALTER TYPE payment_method ADD VALUE 'card_machine'; END IF; END $$`,
